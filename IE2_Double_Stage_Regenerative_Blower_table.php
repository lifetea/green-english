...
<?php
if($lan == "zh_CN"){
   require 'seo/cn/IE2_Double_Stage_Regenerative_Blower_table.php';
}else{
   require 'seo/en/IE2_Double_Stage_Regenerative_Blower_table.php';
}

 ?>

<title>2RB <?php  echo $_GET['series'];?> IE2 Series <?php echo _('Regenerative Blower')?></title>
<meta name="description" content="<?php echo _('IE2 blower,EFF1 blower,IE3,Regenerative blower,Ring blower with NSK,SKF bearing high temperaturer grease,The world leader in manufacturer,export to more than 100 countries by Greenco.')?>" />
<meta name="keywords" content="<?php echo _('IE2,IE3,EFF1,EFF2,Side channel blower,Regenerative blower,Ring blower,single stage,double stage,multi stage blower,high efficiency blower,2RB,3RB,4RB series,GREENCO')?>" />
...
<div id="Side_Channel_Blower_2RB_Single_Stage_all" >
   <div id="Side_Channel_Blower_2RB_Single_Stage">
      ...
      <?php require 'inc/prod-left.php';?>
       <div class="right">
             <h5><i>2RB <?php echo  $_GET['series'];?> IE2 <?php echo _('Series Ordering Data and Performance Data');?></i> ▪ <?php echo _('Side Channel Blower');?></h5>
             <table  cellspacing="0" cellpadding="0" id="data">
                 ...
                 <tr class="two"><td>一</td><td style="width:100px;">Hz</td><td style="width:65px;">Kw</td><td>m³/h</td><td>mbar</td><td style="width:150px;">mbar</td></tr>
                   <?php  while($_rows=mysql_fetch_array($_result)){?>
                             <tr class="content" onclick="window.location.href='IE2_Double_Stage_Regenerative_Blower_content.php?series=<?php echo $_rows['type'];?>&output=<?php echo $_rows['output'];?>' "><td><strong style="font-size:12px;"><a href="IE2_Double_stage_Regenerative_Blower_content.php?series=<?php echo $_rows['type'];?>&output=<?php echo $_rows['output'];?>"><?php  echo $_rows['type'];?></a></strong></td><td style="width:100px;"><?php  echo $_rows['Fre_quency'];?></td><td style="width:65px;"><?php  echo $_rows['output'];?></td><td><?php  echo $_rows['maximum_airflow'];?></td><td><?php  echo $_rows['maximum_vacuum'];?></td><td><?php  echo $_rows['maximum_pressure'];?></td></tr>
                  <?php }?>
                </table>
                 <h3><a href="<?php echo _('upfile')?>/IE2_series/Side channel blower 2rb<?php echo substr($_GET['series'],0,3)?>-IE2.pdf" target="_blank" >Download Side Channel Blower 2RB<?php echo substr($_GET['series'],0,3)?> Series(PDF-<?php $type=$_GET['series'];$filename ='upfile/IE2_Series/side channel blower 2RB'.$type.'-IE2.PDF'; echo floor(filesize($filename)/1024);?>KB)</a></h3>
       </div>
   </div>
</div>
<!-- 应用内容end -->
