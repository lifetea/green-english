...
$_result=mysql_query("select type,sound_level,Fre_quency,output,maximum_airflow,maximum_vacuum,maximum_pressure,weight,phases,Inlet_outlet,A,A1,B,B1,C,C1,D,E,F,F1,F11,G,H,H1,J,K,L,M,N,N1,O,P,P1,Q,ϕR,S,S1,S2,S3,T,T1,U,V,V1,V1_,V3,V3_,V_PIE,V_1,V_PIE1,α,Y_Z,Y,X_Holes,ϕX,W,G1,V2  from product where  type like '%{$_GET['series']}%' and output={$_GET['output']} ");
   ...
   <div id="Side_Channel_Blower_2RB_Single_Stage">
       ...
       <div class="right">
              <div id="left">
                ...
                <tr><td><strong>Inlet/outlet:</strong> <span><?php echo $_rows['ϕR'];?></span>(inch)</td><td><strong>Sound:</strong> <span><?php echo $_rows['sound_level'];?></span> dB(A)</td><td>
                <tr><td><strong>Dimension(L*W*H):</strong> <span><?php  echo $_rows['F1'].'×'.$_rows['A'].'×'.($_rows['B']+$_rows['B1']);?>
                	                         </span> (mm)</td><td>
                </table>
             </div>
             <div id="right">
             ...
             <!-- 尺寸图判断 -->
             <?php
                 echo '<a rel="shadowbox" href="product_image/dimensions/3RB 350-1_big.jpg"><img alt="Dimension for side channel blower '.$_rows['type'].' " class="border" src="product_image/dimensions/3RB 350-1_small.jpg"  title="Click to Enlarge"/></a>';
             ?>

             <!-- 技术参数判断 -->
             <?php
		            echo    '<table class="data"  cellspacing="0" cellpadding="0" style="margin-top:30px;">'.
		              '<tr class="one"><td style="font-style:Italic;">Type</td><td style="font-style:Italic;">Phases</td><td>A</td><td>B</td><td>C</td><td>D</td><td>E</td><td>F</td><td>F\'</td><td>G</td><td>G\'</td><td>H</td><td>H\'</td></tr>'.
		              '<tr><td>'.$_rows['type'].'</td><td>'.$_rows['phases'].'</td><td>'.$_rows['A'].'</td><td>'.$_rows['B'].'</td><td>'.$_rows['C'].'</td><td>'.$_rows['D'].'</td><td>'.$_rows['E'].'</td><td>'.$_rows['F'].'</td><td>'.$_rows['F1'].'</td><td>'.$_rows['G'].'</td><td>'.$_rows['G1'].'</td><td>'.$_rows['H'].'</td><td>'.$_rows['H1'].'</td></tr>'.
		              '<tr class="one"><td style="font-style:Italic;">Type</td><td style="font-style:Italic;">Phases</td><td>K</td><td>L</td><td>P</td><td>ϕR</td><td>V1</td><td>V2</td><td>W</td><td>ϕX</td><td>Y</td><td>Y×Z</td><td></td></tr>'.
		              '<tr><td>'.$_rows['type'].'</td><td>'.$_rows['phases'].'</td><td>'.$_rows['K'].'</td><td>'.$_rows['L'].'</td><td>'.$_rows['P'].'</td><td>'.$_rows['ϕR'].'</td><td>'.$_rows['V1'].'</td><td>'.$_rows['V2'].'</td><td>'.$_rows['W'].'</td><td>'.$_rows['ϕX'].'</td><td>'.$_rows['Y'].'</td><td>'.$_rows['Y_Z'].'</td><td></td></tr>'.
		             '</table>';
             ?>
