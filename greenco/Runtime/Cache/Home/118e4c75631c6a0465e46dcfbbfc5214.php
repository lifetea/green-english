...
@define('feifa',ture); require 'setLan.php'; require 'inc/common.php'; global $_pagenum;global $_pagesize;if($lan == "zh_CN"){ _page("SELECT id  FROM chinese_news", 15); $_result=mysql_query("select id,title,time,content from chinese_news  where  type='支持' order by time desc limit $_pagenum,$_pagesize"); }else{ _page("SELECT id  FROM english_news", 15); $_result=mysql_query("select id,time,title,content  from english_news where type='support' order by time desc limit $_pagenum,$_pagesize"); } ?>
<!DOCTYPE html>
...
<div class="container-fuild">
   <div class="container support">
       <img src="image/support.jpg" style="width:100%;margin:1em auto 0;border:1px solid #bbb;" alt="Greenco side channel blower news"/>
       <div class="row">
       			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; echo ($vo["title"]); ?> <?php echo ($vo["time"]); endforeach; endif; else: echo "" ;endif; ?>	
	       <div class="col-md-3">
	             <h3 style="font-family:tahoma;  text-indent: 10px; background: #666; font-size: 14px;  font-weight: 600;height: 32px;color: #fff;line-height: 32px;border: 1px solid #555;"><?php echo (L("HEADER_NEWS_CENTER")); ?></h3>
	                 <ul>
		                  <li style="margin-top:-2px;"><a href="news_company.php">企业动态</a></li>
		                  <li><a href="news_Industry.php">行业新闻</a></li>	
		                  <li style="margin-top:-2px;"><a href="support"><?php echo _('Technical Support');?></a></li>
	                  </ul>
	       </div>
	       <div class="col-md-9" style="border-left: 1px solid #ccc;">
	       <h3 style="color:#eee;text-indent:5px;font-size:14px;font-family:tahoma;font-family:tahoma;  text-indent: 10px; background: #666; font-size: 14px;  font-weight: 600; ;height: 32px;color: #fff;line-height: 32px;border: 1px solid #555; "><?php echo _('Technical Support');?></h3>
	       <ul>
	       ...
	       <?php  while($_rows=mysql_fetch_array($_result)){?>
	            <li class="mypagination"><p></p><p><strong><a href="support_content.php?id=<?php echo $_rows['id'];?>"><?php echo $_rows['title'];?></a></strong></p>
	            	<p>
	<?php  if($lan == "zh_CN"){ echo _content($_rows['content'],100); }else{ echo _content($_rows['content'],200); } ?>
