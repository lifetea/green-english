...
<title>消音器|消音装置|高压风机|<?php echo  $_GET['type'];?>技术参数|浙江格凌实业有限公司</title>
<meta name="description" content="格凌消音器用于降低或者减少高压风机,漩涡气泵,旋涡风机,真空泵的噪音,可以使用于真空或者压缩气体应用." />
<meta name="keywords" content="消音器,消音装置,降音装置,高压风机,漩涡气泵,旋涡风机,真空泵,2BX4,2RX4011,2RX4012,2RX4013,2RX4014,GREENCO 格凌,浙江格凌,格凌实业,浙江格凌实业有限公司" />
...
<style>
	...
	#Side_Channel_Blower_2RB_Single_Stage .right{height:890px;float:left;width:700px;border-left:1px solid #ccc;padding:0 0 100px 15px;}
    ...
    table.data span{vertical-align:bottom;font-size:9px;color:#333;}
  	table.data tr.one{background:url(image/table_bg.png) repeat-x;font-size:13px;}
...
<div id="Side_Channel_Blower_2RB_Single_Stage_all" >
   <div id="Side_Channel_Blower_2RB_Single_Stage">
       ...
       <div class="right">
             <div id="left">
                <h4>Silencer <?php echo $_rows['type'];?> Technical Data</h4>
                <table style="font-size:12px;">
                   <tr><td><strong>Model: </strong><span><?php echo $_rows['type'];?></span></td><td>
                </table>
                <h5 style="background:#fff url(image/download1.png) no-repeat ;color:#333;border:none;line-height:45px;text-indent:36px;margin-top:180px;font-size:14px;"><a href="upfile/Side channel blower Silencer.pdf" target="_blank" style="color:#1850a3;">Download PDF Silencer (532 KB)</a></h5>
             </div>
             <div id="right">
                <img src="product_image/product_content_big/silencer.jpg" alt="<?php echo $_rows['type'];?> side channel blower silencer image and picture"/>
             </div>


             <h5>Dimension for Silencer <?php echo $_rows['type'];?></h5>
             <!-- 尺寸图判断 -->
             <?php
                    echo   '<a rel="shadowbox" href="product_image/Dimensions_Silencer/silencer_2BX4_big.jpg"><img alt="Dimension for Silencer '.$_rows['type'].' " class="border" src="product_image/Dimensions_Silencer/silencer_2BX4_small.jpg"  title="Click to Enlarge"/></a>';
             ?>

             <!-- 技术参数判断 -->
             <?php
		            echo    '<table class="data"  cellspacing="0" cellpadding="0" style="margin-top:30px;">'.
		              '<tr class="one"><td style="font-style:Italic;">Type</td><td>a</td><td>b</td><td>c</td><td>d</td><td>e</td><td>f</td><td>g</td><td>h</td><td>i</td><td>j</td><td>k</td></tr>'.
		              '<tr><td>'.$_rows['type'].'</td><td>'.$_rows['a'].'</td><td>'.$_rows['b'].'</td><td>'.$_rows['c'].'</td><td>'.$_rows['d'].'</td><td>'.$_rows['e'].'</td><td>'.$_rows['f'].'</td><td>'.$_rows['g'].'</td><td>'.$_rows['h'].'</td><td>'.$_rows['i'].'</td><td>'.$_rows['j'].'</td><td>'.$_rows['k'].'</td></tr>'.
		             '</table>';
             ?>
