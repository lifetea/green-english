...
$_result=mysql_query("select id,time,title,content  from english_news where type='company' order by time DESC ");
?>
...
<div id="Side_Channel_Blower_2RB_Single_Stage_all" >
   <div id="Side_Channel_Blower_2RB_Single_Stage">
       ...
       <div class="right">
       <h4 style="margin:18px 0 10px 0;color:#444;font-size:18px;font-family:tahoma;">Company News</h4>
       <ul>
       <?php  while($_rows=mysql_fetch_array($_result)){?>
            <li class="mypagination"><p></p><p><strong><a href="news_content.php?id=<?php echo $_rows['id'];?>"><?php echo $_rows['title'];?></a></strong></p><p><?php echo _title($_rows['content']);?></p><p class="more"><?php echo $_rows['time'];?></p></li>
       <?php }?>
       </ul>
