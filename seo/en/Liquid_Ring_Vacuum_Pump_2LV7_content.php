...
<title><?php echo $_GET['type'];?> Liquid Ring Vacuum Pump Technical Data</title>
...
<meta name="keywords" content="Liquid Ring Vacuum Pump,Side channel blower,Regenerative blower,Ring blower,Air blower,Vacuum pump,single stage,double stage,multi stage blower,2RB,3RB,4RB series,GREENCO" />
...
<style>
    ...
    #Side_Channel_Blower_2RB_Single_Stage .right #left h4{border-bottom:1px dashed #ccc;color:#1850a3;font-size:21px;font-family:Microsoft YaHei;height:50px;line-height:50px;}
  	...
  	table{background:#fff;font-size:12px;font-family:microsoft yahei;color:#444;border:none;margin-top:10px;width:400px;}
    #Side_Channel_Blower_2RB_Single_Stage .right #left strong{font-size:12px;}
   ...
   <div id="Side_Channel_Blower_2RB_Single_Stage">
       ...
       <div class="right">
              <div id="left">
                ...
                <tr><td><strong>Model: </strong><span><?php echo $_rows['type'];?></span></td><td><strong>Freq:</strong> <span><?php echo $_rows['Fre_quency'];?></span> (Hz)</td></tr>
                <tr><td><strong>Power:</strong> <span><?php echo $_rows['output'];?></span> (Kw)</td><td><strong>Airflow:</strong> <span><?php echo $_rows['maximum_airflow'];?></span> (m³/h)</td></tr>
                <tr><td><strong>TH.CL:</strong> <span>IP55</span></td><td><strong>IN.CL:</strong> <span>F or H</span></td></tr>
                <tr><td><strong>N.weight:</strong> <span><?php echo $_rows['weight'];?></span> (kg)</td><td><strong>Inlet/outlet:</strong> <span><?php echo $_rows['Inlet_outlet'];?></span> (inch)</td></tr>
                <tr><td><strong>Rated voltage:</strong> <span><?php echo $_rows['rated_voltage'];?></span>(V)</td><td><strong>Sound:</strong> <span><?php echo $_rows['sound'];?></span> dB(A)</td></tr>
                <tr><td><strong>Oper.Liquid qty:</strong> <span><?php echo $_rows['oper'];?></span> (m³/h)</td><td><strong>Rated Current:</strong> <span><?php echo $_rows['rated_current'];?></span>(A)</td></tr>
                <tr><td><strong>Dimension:</strong> <span><?php echo $_rows['Dimension'];?></span> (mm)</td><td><strong>Service Fator<span style="vertical-align:super;color:#555;font-size:8px;">1)</span>:</strong> <span><?php echo $_rows['fator'];?></span></td></tr>
                </table>
             </div>
             <div id="right">
                <img src="product_image/product_content_big/<?php echo  $_rows['type'];?>.png" alt="<?php echo $_rows['type'];?> side channel blower image and picture"/>
             </div>


             <h5>Dimension for Liquid Ring Vacuum Pump <?php echo $_rows['type'];?></h5>
             <!-- 尺寸图判断 -->
             <?php
                    echo       '<a rel="shadowbox" href="product_image/Demensions_2LV7/2LV7_big.jpg"><img alt="Dimension for side channel blower '.$_rows['type'].' " class="border" src="product_image/Demensions_2LV7/2LV7_small.jpg"  title="Click to Enlarge"/></a>';
             ?>

             <!-- 技术参数判断 -->
             <?php
		            echo    '<table class="data"  cellspacing="0" cellpadding="0" style="margin-top:30px;">'.
		              '<tr class="one"><td>Model</td><td>A</td><td>A1</td><td>A2</td><td>B</td><td>C</td><td>D</td><td>E</td><td>F</td></tr>'.
		              '<tr><td>'.$_rows['type'].'</td><td>'.$_rows['A'].'</td><td>'.$_rows['A1'].'</td><td>'.$_rows['A2'].'</td><td>'.$_rows['B'].'</td><td>'.$_rows['C'].'</td><td>'.$_rows['D'].'</td><td>'.$_rows['E'].'</td><td>'.$_rows['F'].'</td></tr>'.
		              '<tr class="one"><td>Model</td><td>G</td><td>H</td><td>I</td><td>J</td><td>K</td><td>L</td><td>M</td><td></td></tr>'.
		              '<tr><td>'.$_rows['type'].'</td><td>'.$_rows['G'].'</td><td>'.$_rows['H'].'</td><td>'.$_rows['I'].'</td><td>'.$_rows['J'].'</td><td>'.$_rows['K'].'</td><td>'.$_rows['L'].'</td><td>'.$_rows['M'].'</td><td></td></tr>'.
		             '</table>';
             ?>
